...
def chat(access_key):
    ...
    # Debug: Verificar qué ejercicios se están recuperando
    logger.debug(f"Ejercicios recuperados para prompt_id {prompt_id}: {exercises_from_db}")
        ...
        if not all([student_email, topic, prompt_content]):
            ...
            if prompt_id:
                ...
                if added_exercises > 0:
                    ...
                    success_message += ". No se agregaron ejercicios predefinidos."
                    
                logger.debug(f"Prompt creado: ID={prompt_id}, AccessKey={access_key}, Ejercicios={added_exercises}")
            else:
